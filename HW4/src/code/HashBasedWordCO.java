...
public class HashBasedWordCO {
	...
	public HashSet<String> getUniqueWords(String text) {
		...
		for (String sentence : sentences) {
			for (String word : sentence.split("\\s+")) {
				...
				// the minWordLen
				if(word.length() >= minWordLen && !isIgnored(word) ) {
					uWords.put(word);
				}
			}
		}
	...
	private boolean isIgnored(String word) {
		for(int i=0; i < ignoreList.length ; i++) {
			if(ignoreList[i].equalsIgnoreCase(word)) return true;
		}
		return false;
	}
	...
	public void fillCoMat(String text, String[] keyWords) {
		...
		for(String sentence : sentences) {
			...
			for(int i=0; i < words.length ; i++) {
				if(coMat.get(words[i]) != null) {
					for(int j=i-windowSize; j <=i+windowSize ; j++) {
						if(i!= j && j < words.length && j >= 0) {
							coMat.get(words[i]).increment(words[j]);
						}
					}
				}
			}
