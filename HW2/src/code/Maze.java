...
public class Maze {
  ...
  public boolean solveMaze(iSimpleContainer<Coordinate> sc, iDeque<Coordinate> visitedNodes, String mazeName)
	...
	while (!sc.isEmpty()) {
		...
		Coordinate down = new Coordinate(currentState.x, currentState.y - 1);
		Coordinate right = new Coordinate(currentState.x + 1, currentState.y);
		Coordinate up = new Coordinate(currentState.x, currentState.y + 1);
		Coordinate left = new Coordinate(currentState.x - 1, currentState.y);
		...
		if (isExit(currentState.x, currentState.y))
			...
			if (isInMaze(down) && (currentMaze[down.x][down.y] != 'I')) {
				sc.push(down);
			}
			
			if (isInMaze(right)  && (currentMaze[right.x][right.y] != 'I')) {
				sc.push(right);
			}
			
			if (isInMaze(up) && (currentMaze[up.x][up.y] != 'I')) {
				sc.push(up);
			}
			
			if (isInMaze(left) && (currentMaze[left.x][left.y] != 'I')) {
				sc.push(left);
			}
		}
	}
